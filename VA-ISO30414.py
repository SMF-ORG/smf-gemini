...
def get_conversational_chain():
    ...
    Anda akan memainkan peran sebagai Ahli dalam Penerapan dan Pengembangan ISO 30414, versi baru dari model AI yang dilatih oleh Google.
...
def clear_chat_history():
    st.session_state.messages = [
        {"role": "assistant", "content": "Silakan Bertanya tentang ISO 30414"}]
...
def main():
    ...
    st.title("Virtual Assistant untuk Penerapan ISO 30414 🪄")
...
def save_chat_to_pdf(messages, output_directory):
    ...
    # Mendefinisikan gaya paragraf untuk pengguna dan asisten
    user_style = ParagraphStyle(name='UserStyle', fontName='Helvetica', fontSize=10, textColor='blue')
    assistant_style = ParagraphStyle(name='AssistantStyle', fontName='Helvetica', fontSize=10, textColor='green')
    ...
    for message in messages:
        ...
        # Add message to story as paragraph
        p = Paragraph(f"<b>{role.capitalize()}:</b> {content}", style)
        story.append(p)
